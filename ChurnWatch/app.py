...
def build_results(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if MODEL_INFO["enabled"]:
        try:
            ...
            model_df = pd.DataFrame({
                ...
                "Promo Code Used": df.get("promo_code_used", "No"),
                "Payment Method": df.get("payment_method", "Unknown"),
            ...
            X_pred, _, _ = preparar_features(model_df, scaler=MODEL_SCALER, fit_scaler=False)
            if MODEL_FEATURES:
                X_pred = X_pred.reindex(columns=MODEL_FEATURES, fill_value=0)

            probs = (MODEL.predict_proba(X_pred)[:, 1] * 100).round().astype(int)
    ...
    df["__model_mode"] = MODEL_INFO["mode_label"] if used_ml else "Modo heurístico"
    return df
...
# 1. Procesar archivo subido → guardar resultados en Store
@app.callback(
...
def process_upload(contents, filename):
    ...
    return (
        records,
        {"display": "none"},          # ocultar upload
        {"display": "block"},         # mostrar resultados
        filename,
...
# 4. Renderizar filas de la tabla (filtro + busqueda)
@app.callback(
...
# 5. Abrir modal al hacer clic en una fila
@app.callback(
...
def open_modal(n_clicks_list, records):
    ...
    # Identificar que fila fue clickeada
