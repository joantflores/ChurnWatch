...
PERCENTIL_UMBRAL = 0.25   # percentil 25  →  Previous Purchases <= 13

# Columnas originales del dataset que se necesitan
COLUMNAS_REQUERIDAS = [
    "Customer ID",
...
# Columnas categóricas que se codificarán con One-Hot Encoding
COLUMNAS_OHE = [
    "Frequency of Purchases",
    "Subscription Status",
    "Discount Applied",
...
def construir_churn(
    ...
    Regla de negocio (proxy variable):
        Churn = 1  si el cliente cumple LAS TRES condiciones:
    ...
    if verbose:
        ...
        cond_subs = (df["Subscription Status"] == "No").sum()
        ...
        print(f"    Sin suscripción              : {cond_subs:,}")
        print(f"    Las 3 condiciones (Churn=1)  : {n_churn:,}\n")
